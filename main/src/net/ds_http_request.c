...
/**
 * @brief 该接口废弃，建议更改为sntp接口使用，可参考ds_sntp.c文件
 * 
 */
void http_time_get(){
    //http client配置
...
void http_weather_get(){
    ...
    if(city != NULL){
        ...
        strcpy(count_url,"http://api.seniverse.com/v3/weather/now.json?key=SmazqPcltzTft-X3v&location=guangzhou&language=zh-Hans&unit=c");
    }
    //http client配置
    esp_http_client_config_t config = {
        .method = HTTP_METHOD_GET, //get请求
...
/**
 * @brief 该接口废弃，B站请求需使用https,需要用可以参考ds_https_request.c接口使用
 * 
 */
void http_fans_get(){
    char url[100];
